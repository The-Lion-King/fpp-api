...
const Context: ContextInterface = {
  ...
  initialize(params: ContextParams): void {
    ...
    if (missing.length) {
      throw new FppErrors.FppError(
        `Cannot initialize Fpp API Library. Missing values for: ${missing.join(
          ', ',
        )}`,
      );
    }
  ...
  throwIfUninitialized(): void {
    if (!this.API_KEY || this.API_KEY.length === 0) {
      throw new FppErrors.UninitializedContextError(
  ...
  throwIfPrivateApp(message: string): void {
    if (Context.IS_PRIVATE_APP) {
      throw new FppErrors.PrivateAppError(message);
    }
  },
};
